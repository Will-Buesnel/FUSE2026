...
from models.local_stats import AdaptiveMetropolisHastings, InvalidProposal, MCMCStop, lengthscale_func_2d
from utils import plot_mixing, plot_traces, set_rc_params, get_path_to_data_results_dir, get_path_to_data_processed_dir, plot_df, save_pred_samples_to_pt, open_pred_samples_as_df, df_to_tensor_dict
...
def run_inference_MCMC(simulator: Simulator, obs=None, warmup_steps=100, num_samples=1000, num_chains=1, adapt_start=800, **kwargs):
    ...
    kernel = AdaptiveMetropolisHastings(model=model, target_accept_prob=0.234, adapt_start = adapt_start, c0=torch.eye(simulator.param_df.shape[0]+2, dtype=torch.float64)*1e-4, init_step_size=1e-4)  # use an adaptive Metropolis-Hastings kernel for MCMC inference
...
def generate_standard_simulator(start_idx=0, stop_idx=70000):
    ...
    initial_voltage = sim.exp_df["Voltage(V)"].iloc[0]  # get the initial voltage from the experiment data
...
def save_and_run_bayesian_mc_infer(warmup_steps: int = 200, samples: int = 200, chains: int = 1,
                                    adapt_start: int = 300, start_idx: int = 334000, stop_idx: int = 350000):
    ...
    save_run(warmup_steps=warmup_steps, samples=samples, chains=chains, filename_prefix=filename_pref, start_idx=start_idx, stop_idx=stop_idx, adapt_start=adapt_start)  # run the inference and save the samples and diagnostics
...
if __name__ == "__main__":
    ...
    filename_pref = save_and_run_bayesian_mc_infer(warmup_steps=500, samples=500, chains=3, adapt_start=400, start_idx=start_idx, stop_idx=stop_idx)  # run the inference and save the samples and diagnostics
