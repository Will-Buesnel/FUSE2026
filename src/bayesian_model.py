...
def run_inference_MCMC(simulator: Simulator, obs=None, warmup_steps=100, num_samples=1000, **kwargs):
    from pyro.infer import MCMC
...
def plot_mixing(samples: pd.DataFrame, param_names):
    ...
    for i, param in enumerate(param_names): 
        ...
        for chain in range(1, num_chains+1):
            ...
            print(f"Chain {chain} samples for {param}: {chain_samples.values[:5]}...")  # print first 5 samples for debugging
...
def _convert_pred_samples_to_df(samples_by_chain, drop_index=True):
    ...
    num_chains = len(samples_by_chain)
    num_samples = samples_by_chain[0][parameter_names[0]].shape[0]
    chain_column = np.repeat(np.arange(1, num_chains+1), num_samples)
    iteration_col = np.tile(np.arange(1, num_samples+1), num_chains)

    data = {"Chain": chain_column, "Iteration": iteration_col}

    for parameter in parameter_names:

        v_stacked_param_all_chains = np.concatenate(
            [
            np.asarray(samples_by_chain[chain][parameter].detach().numpy()) for chain in range(num_chains)
        ]
        )
        data[parameter] = list(v_stacked_param_all_chains)
...
def open_pred_samples_as_df(filename, drop_index=True):
...
def generate_sample_test(num_samples=NUM_SAMPLES, warmup_steps=WARMUP_STEPS, num_chains=NUM_CHAINS):
    ...
    lp_cell = Cell(
        ...
        c_p = 887, # J kg^-1 K^-1
        rho = 2682, # kg m^-3,
        entropy_coeff_func = entropy_coeff_func
    )
